...
- 任何上游同步都不得削弱、移除或擅自改变 [docs/fork-features.md](docs/fork-features.md) 中仍然有效的二开功能；即使 Git 自动合并成功，也必须保留这些功能的完整用户行为和适用边界。
- 如果上游改动与现有二开功能确实无法同时保留，或者继续合并必须削弱、移除或改变二开行为，必须立即暂停相关合并，并在修改前向用户说明冲突、具体影响和可选处理方案；只有取得用户明确确认后才能继续，禁止自行选择上游行为覆盖二开行为。
