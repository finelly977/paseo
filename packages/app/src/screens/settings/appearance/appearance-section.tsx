...
function getThemeLabel(t: TFunction, value: BuiltInThemePreference): string {
  const labelKeys: Record<BuiltInThemePreference, string> = {
...
const PRIMARY_THEMES: readonly BuiltInThemePreference[] = ["light", "dark", "auto"];
const DARK_VARIANT_THEMES: readonly BuiltInThemePreference[] = [
  "zinc",
  "midnight",
  "claude",
  "ghostty",
...
function ThemeRow({
  ...
  const { t } = useTranslation();
  const builtInValue = value === PLUGIN_THEME_PREFERENCE ? "auto" : value;
        ...
        <DropdownMenuContent side="bottom" align="end" width={200}>
          {PRIMARY_THEMES.map((themeValue) => (
            <ThemeMenuItem
              ...
              selected={selectedPluginTheme === null && builtInValue === themeValue}
              onChange={onChange}
            />
          ))}
          <DropdownMenuSeparator />
...
export function AppearanceSection() {
  ...
  const contributedThemes = useContributedThemes();
  ...
  const handleThemeChange = useCallback(
    (theme: BuiltInThemePreference) => {
      void updateSettings({ theme, pluginThemeId: null });
    },
    [updateSettings],
  );
      ...
      <SettingsSection title={t("settings.appearance.theme.title")}>
        <View style={settingsStyles.card}>
          <ThemeRow
            ...
            pluginThemes={contributedThemes.options}
            selectedPluginTheme={contributedThemes.selected}
            onChange={handleThemeChange}
            onSelectPluginTheme={contributedThemes.select}
          />
        </View>
      </SettingsSection>
