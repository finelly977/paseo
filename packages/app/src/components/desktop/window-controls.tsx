...
const WINDOW_CONTROLS_Z_INDEX = 50;
...
const passiveGlyphMapping = (theme: Theme) => ({ color: theme.colors.foregroundExtraMuted });
const destructiveGlyphMapping = (theme: Theme) => ({ color: theme.colors.destructiveForeground });
      ...
      return (
        ...
        >
          <ThemedWindowControlGlyph
            ...
            uniProps={closeHighlighted ? destructiveGlyphMapping : passiveGlyphMapping}
          />
        </View>
      );
    },
...
export function DesktopWindowControls() {
  ...
  const minimize = useCallback(() => {
    void minimizeDesktopWindow().catch((error) => {
      console.error("[桌面窗口] 最小化失败", error);
    });
  }, []);
  const toggleMaximize = useCallback(() => {
    void toggleDesktopMaximize().catch((error) => {
      console.error("[桌面窗口] 切换最大化状态失败", error);
    });
  }, []);
  const close = useCallback(() => {
    void closeDesktopWindow().catch((error) => {
      console.error("[桌面窗口] 关闭失败", error);
    });
  }, []);
...
const styles = StyleSheet.create((theme) => ({
  controls: {
    ...
    top: 0,
    right: 0,
    zIndex: WINDOW_CONTROLS_Z_INDEX,
    flexDirection: "row",
  },
  control: {
    height: "100%",
