...
interface SourceToken extends HighlightToken {
  key: string;
}
  ...
  const lines = useMemo(() => {
    if (presentation === "highlighted")
      return highlightCode(content, filename).map((tokens, index) => ({
        number: index + 1,
        tokens: buildSourceTokens(tokens),
      }));
    return content.split("\n").map((text, index) => ({
      number: index + 1,
      tokens: [{ key: `0:${text.length}`, text, style: null }],
    }));
    ...
    <View style={styles.line}>
      ...
      <Text selectable style={styles.text}>
        {line.tokens.map((token) => (
          <Text key={token.key} style={syntaxTokenStyleFor(token.style)}>
            {token.text}
          </Text>
        ))}
      </Text>
...
function buildSourceTokens(tokens: HighlightToken[]): SourceToken[] {
  let offset = 0;
  return tokens.map((token) => {
    const start = offset;
    offset += token.text.length;
    return { ...token, key: `${start}:${offset}:${token.style ?? "plain"}` };
  });
}
