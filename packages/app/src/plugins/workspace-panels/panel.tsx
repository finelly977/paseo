...
function PluginPanelBody({ theme }: { theme: PluginTheme }) {
  ...
  invariant(target.kind === "plugin", "PluginPanel 需要插件目标");
  ...
  if (!runtime) {
    return <PluginPanelUnavailable message="插件宿主已离线。" />;
  }
...
function PluginPanelUnavailable({ message = "此插件面板不可用。" }: { message?: string }) {
  return (
