    ...
    const installed = catalog.flatMap((entry) => {
      ...
      try {
        ...
        if (installation.clientSide) {
          if (!options.client) throw new Error("插件客户端运行时不可用");
          let clientCleanup;
          try {
            clientCleanup = startPluginClientSide(installation, options.client);
          } catch (error) {
            queryClient.clear();
            disposeFailedInstallation(installation);
            throw error;
          }
        ...
        console.warn(`[插件] 无法加载 ${serverId}/${entry.id}`, error);
        return [];
      }
    });
  ...
  private dispose(plugin: InstalledPlugin): void {
    ...
    try {
      void Promise.resolve(plugin.cleanup()).catch((error) => {
        console.warn(`[插件] 清理 ${plugin.serverId}/${plugin.id} 失败`, error);
      });
    } catch (error) {
      console.warn(`[插件] 清理 ${plugin.serverId}/${plugin.id} 失败`, error);
    }
...
function disposeFailedInstallation(
  plugin: Pick<InstalledPlugin, "id" | "serverId" | "cleanup">,
): void {
  try {
    void Promise.resolve(plugin.cleanup()).catch((error) => {
      console.error(`[插件] 清理启动失败的插件 ${plugin.serverId}/${plugin.id} 失败`, error);
    });
  } catch (error) {
    console.error(`[插件] 清理启动失败的插件 ${plugin.serverId}/${plugin.id} 失败`, error);
  }
