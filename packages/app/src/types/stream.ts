...
export function createUserMessage(input: UserMessageInput): UserMessageItem {
  ...
  return {
    ...
    ...(input.optimistic ? { optimistic: true as const } : {}),
...
type UserMessageMatchPolicy = "canonical-incoming" | "handoff" | "live-provider-echo";
...
function matchesUserMessage(
  ...
  if (
    existing.optimistic &&
    (incoming.clientMessageId === existing.id ||
      incoming.messageId === existing.id ||
      (incoming.clientMessageId === undefined && incoming.text === existing.text))
  ) {
    return true;
  }
  if (
...
function mergeUserMessage(
  ...
  const hasCanonicalIdentity =
    ...
    incoming.timelineCursor !== undefined ||
    existing.timelineCursor !== undefined;
  return createUserMessage({
    ...presentation,
    id: shouldAdoptProviderId ? incoming.id : presentation.id,
...
function hasSameUserMessagePresentation(
  ...
  return (
    ...
    existing.providerImages === merged.providerImages &&
...
function produceUserMessage(
  ...
  if (!existing || existing.kind !== "user_message") {
    throw new Error("用户消息合并错误地匹配到了非用户消息");
  }
  const merged = mergeUserMessage(existing, incoming, presentationPolicy, matchPolicy);
...
export function handoffCreatedAgentUserMessageToStream(params: {
  ...
  const message = params.message.clientMessageId
    ? params.message
    : createUserMessage({ ...params.message, clientMessageId: params.message.id });
...
function reconcileCanonicalUserMessageTurn(
  items: StreamItem[],
  canonical: UserMessageItem,
  turnId: string | undefined,
): StreamItem[] {
...
function applyCanonicalUserMessageToHead(params: {
  tail: StreamItem[];
  head: StreamItem[];
  canonical: UserMessageItem;
  turnId: string | undefined;
  ...
  const { tail, head, canonical, turnId, hasContent, source } = params;
  const reconciled = upsertUserMessageAcrossStream({
    ...
    matchPolicy: source === "live" ? "live-provider-echo" : "canonical-incoming",
  });
  const reconciledTail = reconcileCanonicalUserMessageTurn(reconciled.tail, canonical, turnId);
  const reconciledHead = reconcileCanonicalUserMessageTurn(reconciled.head, canonical, turnId);
  return {
    tail: reconciledTail,
    head: reconciledHead,
    ...
    acknowledgedClientMessageIds: reconciled.location
      ? acknowledgedCanonicalUserMessage(reconciled.location.matched, reconciled.location.message)
      : [],
  };
}

function applyCanonicalUserMessageToTail(params: {
  tail: StreamItem[];
  head: StreamItem[];
  canonical: UserMessageItem;
  turnId: string | undefined;
  ...
  const reconciled = produceUserMessage(
    flushedTail,
    canonical,
    hasContent ? flushedTail.length : null,
    "existing",
    source === "live" ? "live-provider-echo" : "canonical-incoming",
  );
  const reconciledTail = reconcileCanonicalUserMessageTurn(reconciled.items, canonical, turnId);
  return {
    tail: reconciledTail,
    head: flushedHead,
    changedTail: flushedTail !== tail || reconciledTail !== flushedTail,
    changedHead: flushedHead !== head,
    acknowledgedClientMessageIds: acknowledgedCanonicalUserMessage(
      reconciled.matched,
      reconciled.message,
    ),
  };
...
function applyCanonicalUserMessageEvent(params: {
  ...
  const { tail, head, event, timestamp, timelineCursor, unmatchedInsert = "tail", source } = params;
  ...
  const canonical = createUserMessage({
    ...
    timestamp,
    providerImages: event.item.images,
  });
  if (unmatchedInsert === "head") {
    return applyCanonicalUserMessageToHead({
      tail,
      head,
      canonical,
      turnId: event.turnId,
  ...
  return applyCanonicalUserMessageToTail({
    tail,
    head,
    canonical,
    turnId: event.turnId,
...
function applyCanonicalUserMessageEventUnlessTransformed(params: {
  tail: StreamItem[];
  head: StreamItem[];
  event: AgentStreamEventPayload;
  timestamp: Date;
  ...
  if (params.transformedTimelineItems !== undefined) return null;
  return applyCanonicalUserMessageEvent(params);
}
...
export function applyStreamEvent(params: {
  ...
  const transformedTimelineItems =
    event.type === "timeline" ? params.transformTimelineItem?.(event.item) : undefined;
  const canonicalUserResult = applyCanonicalUserMessageEventUnlessTransformed({
    tail,
    head,
    event,
    timestamp,
