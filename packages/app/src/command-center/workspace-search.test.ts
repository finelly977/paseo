...
describe("filterAndRankWorkspaces", () => {
  ...
  it("rejects zero and numbers outside JavaScript's safe integer range", () => {
    const unrelated = workspace({ id: "unrelated", changeRequestNumber: 1 });
    for (const query of ["0", "pr0", "9007199254740992"]) {
      expect(ids([unrelated], query)).toEqual([]);
    }
  });
