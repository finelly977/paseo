...
interface ReplicaInput {
  ...
  timelineCursor: AgentTimelineCursorState | null;
...
function timelineBase(item: StreamItem) {
  const timelineCursor = "timelineCursor" in item ? item.timelineCursor : undefined;
  return {
    id: item.id,
...
function replicaInputsEqual(left: ReplicaInput, right: ReplicaInput): boolean {
  return (
    ...
    (left.timelineCursor === right.timelineCursor ||
      (left.timelineCursor !== null &&
        right.timelineCursor !== null &&
        left.timelineCursor.epoch === right.timelineCursor.epoch &&
...
function selectReplicaInput(session: SessionState, agentId: string | null): ReplicaInput {
  ...
  const workspace = agent
    ...
    : undefined;
  const timelineItems = agentId ? session.agentStreamTail.get(agentId) : undefined;
  return {
    agent,
    workspace,
    project: workspace ? session.projects.get(workspace.projectId) : undefined,
    timelineItems,
    timelineCursor: agentId ? (session.agentTimelineCursor.get(agentId) ?? null) : null,
    timelineHasOlder: agentId ? (session.agentTimelineHasOlder.get(agentId) ?? false) : false,
  };
}
    ...
    timeline:
      input.agent && items
        ? {
            ...
            hasOlder: input.timelineHasOlder,
          }
        : null,
  };
}
