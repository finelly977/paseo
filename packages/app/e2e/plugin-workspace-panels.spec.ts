...
async function runCleanup(label: string, cleanup: () => Promise<unknown>): Promise<void> {
  try {
    await cleanup();
  } catch (error) {
    console.error(`[插件端到端测试] ${label}失败`, error);
  }
}
...
test.describe("plugin workspace panels and Command Center", () => {
  ...
  test("follows workspace, agent, host, compact, and unavailable state", async ({
    ...
    try {
      ...
      await runCleanup("移除主插件", () => primaryClient.removePlugin(PLUGIN_ID));
      await runCleanup("恢复插件设置", () =>
        primaryClient.patchDaemonConfig({
          pluginsEnabled: previousConfig.config.pluginsEnabled ?? false,
        }),
      );
      await runCleanup("关闭主客户端", () => primaryClient.close());
      await runCleanup("清理主工作区", () => primary.cleanup());
      await runCleanup("清理辅助工作区", () => secondary.cleanup());
      await runCleanup("关闭辅助守护进程", () => secondaryDaemon.close());
