...
test("plugin modal adapts its presentation and preserves host contexts", async ({ page }) => {
  ...
  try {
    ...
    await client.removePlugin(PLUGIN_ID).catch((error: unknown) => {
      console.error("[插件宿主界面端到端测试] 清理插件失败", error);
    });
    await client
      .patchDaemonConfig({ pluginsEnabled: previousConfig.config.pluginsEnabled ?? false })
      .catch((error: unknown) => {
        console.error("[插件宿主界面端到端测试] 恢复插件开关失败", error);
      });
    await client.close().catch((error: unknown) => {
      console.error("[插件宿主界面端到端测试] 关闭客户端失败", error);
    });
