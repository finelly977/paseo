...
import { scorePathMatch, type MatchScore } from "@getpaseo/protocol/search/path-match";
...
function rank(entry: TraversedEntry, input: SearchInput): RankedEntry {
  ...
  const rankFields = input.plan.isPathQuery
    ? rankPathMatch(
        pathScore,
        input.plan.searchTerm
          ? scorePathMatch(input.plan.searchTerm, segments.at(-1) ?? "")
          : pathScore,
      )
...
function rankPathMatch(pathScore: MatchScore | null, terminalScore: MatchScore | null): RankFields {
  if (!pathScore || !terminalScore) {
    return {
      matchTier: NO_MATCH_TIER,
  ...
  return {
    matchTier: Math.min(pathScore.tier, 4),
    segmentIndex: NO_SEGMENT_INDEX,
    matchOffset: pathScore.offset,
