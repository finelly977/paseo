...
async function createRealHarness() {
  ...
  try {
    ...
    return {
      ...
      async runScenario(
        ...
        try {
          ...
          await logCleanupFailure("关闭 OpenCode 测试会话", () => session.close());
        }
      },
      async close() {
        try {
          await logCleanupFailure("释放 OpenCode 测试服务租约", () => lease.release());
          await logCleanupFailure("关闭 OpenCode 测试服务", () => manager.shutdown());
          await proxy.close();
          writeFileSync(
    ...
    if (setupManager) {
      await logCleanupFailure("清理 OpenCode 测试服务", () => setupManager.shutdown());
    }
    if (setupProxy) {
      await logCleanupFailure("清理 OpenCode 测试代理", () => setupProxy.close());
    }
    rmSync(runtimeDir, { recursive: true, force: true });
    throw error;
  }
...
async function logCleanupFailure(description: string, cleanup: () => Promise<void>): Promise<void> {
  try {
    await cleanup();
  } catch (error) {
