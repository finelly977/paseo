...
describe("OMP CLI runtime", () => {
  ...
  test("uses the configured timeout while waiting for the ready phase", async () => {
    vi.useFakeTimers();
    const child = createOmpChild({ emitReady: false });
    const startup = createRuntime(child, [], { requestTimeoutMs: 100 }).startSession({
      cwd: "/workspace/project",
    });
    const rejection = expect(startup).rejects.toThrow(
      "OMP RPC request timed out phase=ready elapsedMs=100 timeoutMs=100",
    );
