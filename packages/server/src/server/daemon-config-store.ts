...
interface SupportedMutableConfigPatch {
  ...
  gitAi?: MutableDaemonConfigPatch["gitAi"];
...
function omitGitAiProvidersFromConfig(
  ...
  if (providers.length === 0 || !config.gitAi) {
    return config;
  }
  const gitAi = omitGitAiProviders(config.gitAi, providers);
  return gitAi === config.gitAi ? config : { ...config, gitAi };
}

function omitGitAiProviders(
  gitAi: NonNullable<MutableDaemonConfig["gitAi"]>,
  providers: readonly string[],
): NonNullable<MutableDaemonConfig["gitAi"]> {
  const removedProviderIds = new Set(providers);
  let changed = false;
  ...
  const commitMessage = resetProfile(gitAi.commitMessage);
  const commitReview = resetProfile(gitAi.commitReview);
  return changed ? { ...gitAi, commitMessage, commitReview } : gitAi;
}
...
const RELOADABLE_PATHS = [
  ...
  "daemon.codexProviderInjections",
  "app.baseUrl",
  "agents.providers",
...
const PERSISTED_TO_MUTABLE_PATH = new Map<string, string>([
  ...
  ["daemon.codexProviderInjections", "codexProviderInjections"],
...
function pickSupportedPatchFields(patch: MutableDaemonConfigPatch): SupportedMutableConfigPatch {
  return {
    ...
    ...(patch.metadataGeneration?.providers !== undefined
      ...
      : {}),
    ...(patch.gitAi !== undefined ? { gitAi: patch.gitAi } : {}),
    ...
    ...(patch.codexProviderInjections !== undefined
      ? { codexProviderInjections: patch.codexProviderInjections }
      : {}),
...
function mergeMutableAgentPatch(
  ...
  if (patch.gitAi !== undefined) {
    next["gitAi"] = GitAiConfigSchema.parse(
      deepMerge(
        GitAiConfigSchema.parse(persistedAgents?.gitAi ?? {}),
        patch.gitAi as Record<string, unknown>,
      ),
    );
  }
  if (removeProviders.length > 0 && next["gitAi"] !== undefined) {
    next["gitAi"] = omitGitAiProviders(
      next["gitAi"] as NonNullable<MutableDaemonConfig["gitAi"]>,
      removeProviders,
    );
  }
...
function mergeMutableDaemonPatch(
  ...
  if (patch.codexProviderInjections !== undefined) {
    next.codexProviderInjections = patch.codexProviderInjections;
  }
