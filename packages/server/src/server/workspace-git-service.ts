...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  async fetch(cwd: string): Promise<void> {
    cwd = resolve(cwd);
    const result = await this.deps.runGitFetch(cwd, { onRefSnapshot: () => {} });
    if (result.error) {
  ...
  private async refreshGitSnapshot(
    ...
    target.latestGit = {
      ...
      stagedFileCount: refreshWorktree
        ? checkoutStatus.stagedFileCount
        : (target.latestGit?.stagedFileCount ?? checkoutStatus.stagedFileCount),
      changes: refreshWorktree
        ? checkoutStatus.changes
        : (target.latestGit?.changes ?? checkoutStatus.changes),
