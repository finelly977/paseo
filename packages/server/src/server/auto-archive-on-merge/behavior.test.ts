...
describe("workspace auto-archive behavior", () => {
  ...
  test("does not archive an unarchived workspace again for the same merged PR", async () => {
    const workspace = await createWorkspaceJourney();
    workspace.checkout("merged-then-unarchived");

    await workspace.observe(pullRequest("merged-then-unarchived", "open"));
    await workspace.observe(pullRequest("merged-then-unarchived", "merged"));
    expect(workspace.isActive()).toBe(false);
