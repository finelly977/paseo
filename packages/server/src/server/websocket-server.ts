...
export class VoiceAssistantWebSocketServer {
  ...
  public publishProjectUpdate(update: ProjectUpdate): void {
    for (const session of this.listSessions()) {
      session.emitProjectUpdate(update);
    }
  }
  ...
  private buildServerInfoStatusPayload(session: Session): ServerInfoStatusPayload {
    return {
      ...
      features: {
        ...
        // COMPAT(canonicalSubmittedPrompts): v0.2.6 新增，2027-01-30 后移除能力门控。
  ...
  private maybeHandleBinaryFrame(params: {
    ...
    if (!activeConnection.session.allowsPermission("workspace.write")) {
      this.sendToClient(
        ws,
        wrapSessionMessage({
          type: "status",
          payload: {
            status: "error",
            message: "Session is not authorized for binary frames",
          },
        }),
      );
      return true;
