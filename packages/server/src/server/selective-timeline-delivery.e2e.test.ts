...
test("回退完成按来源能力和订阅范围投递", async () => {
  ...
  expect(passive.messages.filter((message) => message.type === "agent.rewind.response")).toEqual(
    [],
  );
  ...
  expect(unrelated.messages.filter((message) => message.type === "agent.rewind.response")).toEqual(
    [],
  );
  expect(legacy.messages.filter((message) => message.type === "agent.rewind.response")).toEqual([]);
