...
describe("ForgeService", () => {
  ...
  it("batches PR status polls across workspaces into one GraphQL request", async () => {
    ...
    // 已合并拉取请求的检查仍可能被重新运行，慢速轮询也必须读取最新结果。
    now = EXPECTED_GITHUB_SLOW_POLL_MS;
    await vi.advanceTimersByTimeAsync(EXPECTED_GITHUB_SLOW_POLL_MS);
    await flushMicrotasks();
    ...
    expect(statusesA[1]).toEqual(expect.objectContaining({ number: 41, checksStatus: "failure" }));
  ...
  it("reports repository host resolution failures instead of routing them as github.com", async () => {
    const hostError = new Error("host resolution failed");
    const runner = createScriptedRunner([]);
    const service = createGitHubService({
      ttlMs: 0,
      runner: runner.runner,
    ...
    expect(runner.calls).toHaveLength(0);
    expect(errors).toEqual([hostError]);

    subscription?.unsubscribe();
    service.dispose?.();
  });

  it("uses the complete per-target lookup when repository slug resolution fails", async () => {
    const runner = createScriptedRunner([
      currentPullRequestJson(),
      currentPullRequestGithubFactsJson(),
    ]);
  ...
  it("reports malformed successful batch responses as protocol failures", async () => {
    const runner = createScriptedRunner(["{not-json"]);
    const service = createGitHubService({
      ttlMs: 0,
      runner: runner.runner,
      resolveGhPath: async () => "/usr/bin/gh",
    ...
    expect((errors[0] as GitHubCommandError).stderr).toContain("not valid JSON");
    expect((errors[0] as GitHubCommandError).cause).toBeInstanceOf(SyntaxError);
  ...
  it("reports a distinct failure when the rate-limit reset time cannot be loaded", async () => {
    const runner = createScriptedRunner([
      {
      ...
      {
        error: Object.assign(new Error("rate limit endpoint failed"), {
          code: 2,
          stderr: "rate limit endpoint unavailable",
        }),
      },
    ]);
    const service = createGitHubService({
    ...
    expect(errors[0]).toMatchObject({
      name: "GitHubGraphqlRateLimitResetError",
      cause: expect.any(GitHubCommandError),
    });
