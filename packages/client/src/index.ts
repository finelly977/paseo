...
export function createPaseoApi(daemonClient: DaemonClient): PaseoApi {
  ...
  const createAgent = async (
    ...
    const { provider: providerModel, ...runtimeConfig } = agentConfig;
  ...
  return {
    projects: {
      list: (options) => daemonClient.listProjects(options?.requestId),
    },
    workspaces: {
...
function waitForProvidersReady(
  ...
  const poll = async (): Promise<PaseoProviderSnapshotResult> => {
    const snapshot = await daemonClient.getProvidersSnapshot(snapshotOptions);
    const loading = snapshot.entries
      .filter((entry) => entry.status === "loading")
      .map((entry) => entry.provider);
    if (loading.length === 0) return snapshot;
    if (Date.now() - startedAt >= timeoutMs) {
      throw new Error(`等待提供方就绪超时：${loading.join("、")}`);
    }
