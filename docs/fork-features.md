...
- 从已经明确项目和主机的侧栏入口新建会话时，先显示不写入服务端记录的完整草稿工作区外壳：顶部草稿标签、工作区菜单、终端/浏览器入口以及右侧文件和 Git 面板无需先发送一条消息即可使用，中央继续沿用原有居中的空白会话输入区。关闭唯一草稿标签会离开新建页，不会留下无法关闭的空标签；发送首条消息或主动打开终端、浏览器、正文文件等需要持久工作区的操作时才创建工作区并出现在左侧。全局“新建工作区”入口没有明确项目时仍保留项目选择步骤，草稿交接入口也不会被自动创建抢占。
