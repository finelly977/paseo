...
- Host 设置侧栏提供独立的“Codex 服务商”页面，用于保存多个有序的会话级服务商注入配置；该能力不再混在 Agents 页面。每项包含显示名称、`model_provider` 标识、一个或多个模型、API 地址、密钥变量名、运行时环境变量、OpenAI 登录要求、WebSocket 支持及可选的额外 `model_providers` 参数。编辑器使用紧凑的结构化字段、可增删的模型和环境变量行，并只把少见字段留在折叠 JSON 区域。Codex 会话主菜单只显示一个“注入 Codex 服务商”入口，进入后先选择服务商，再选择该服务商下的模型，服务商数量不会拉长主菜单；旧配置没有模型时仍可选择保留会话当前模型。尚未加载的会话会携带所选服务商与模型直接恢复或启动，已加载的会话会先完整释放旧 app-server 再恢复同一原生线程。选择结果随 Paseo 会话持久化，后续自动恢复继续使用同一配置；配置被删除后会明确报错，不会静默回退到其他服务商或用户全局配置。
