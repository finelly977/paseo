...
- 从已经明确项目和主机的侧栏入口新建会话时，会先创建空工作区并直接进入正式工作区外壳；顶部标签、工作区菜单、终端/浏览器入口以及右侧文件和 Git 面板无需先发送一条消息即可使用，中央仍显示居中的空白会话输入区。全局“新建工作区”入口没有明确项目时仍保留项目选择步骤，草稿交接入口也不会被自动创建抢占。
- 工作区支持按加入 Paseo 的先后、名称和自定义顺序排列，默认沿用加入 Paseo 的先后。切换到名称排序时禁用拖动，拖动加入时间顺序中的工作区后自动进入自定义顺序。
- 工作区首次被 Paseo 发现的顺序和手工拖动顺序分别持久化；新加入的工作区目录在“加入 Paseo 时间”和手工拖动后的“自定义顺序”中都直接置顶，同时保留已有目录的相对顺序。未手工排序的工作区内部会话按加入 Paseo 的时间从新到旧排列，新会话即使尚未发送消息也直接出现在顶部，不受标题、最近活动或请求返回顺序影响。加入时间使用主机持久化的工作区创建时间，创建响应、目录快照、实时更新和客户端副本均保留该时间；相同时间按稳定标识排列，改名、继续对话和重新打开应用不会让会话跳位。拖动会话后持久化该工作区的自定义顺序，之后发现的新会话仍插入顶部并保留已知会话的手工相对顺序。会话菜单同时提供两种互斥置顶：全局置顶会把会话提升到跨工作区的“已置顶”分组，工作区内置顶则让会话留在原工作区并排到同组其他会话之前；切换范围时会先取消另一种置顶，两个范围都按最近置顶时间排列。状态视图中的全局置顶同样只显示一次，工作区内置顶不改变状态分组优先级。
