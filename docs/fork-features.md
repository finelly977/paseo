...
- 手机端键盘升降通过同一个布局停靠层同步约束对话正文、插件输入附件和输入框，自动补全浮层会跟随键盘实时调整底边与最大高度；展开长草稿时只压缩可滚动文本区，24 像素发送/停止按钮和其他底部控制始终可见。网页和桌面端发送后继续保持输入焦点，原生端仍按平台习惯收起键盘；这些行为不改变二开输入框尺寸、上下文环和用户可调间距。
...
- 工作区首次被 Paseo 发现的顺序和手工拖动顺序分别持久化；新加入的工作区目录在“加入 Paseo 时间”和手工拖动后的“自定义顺序”中都直接置顶，同时保留已有目录的相对顺序。未手工排序的工作区内部会话按加入 Paseo 的时间从新到旧排列，新会话即使尚未发送消息也直接出现在顶部，不受标题、最近活动或请求返回顺序影响。加入时间使用主机持久化的工作区创建时间，创建响应、目录快照、实时更新和客户端副本均保留该时间；相同时间按稳定标识排列，改名、继续对话和重新打开应用不会让会话跳位。拖动会话后持久化该工作区的自定义顺序，之后发现的新会话仍插入顶部并保留已知会话的手工相对顺序。手动置顶会话时，会话仍留在所属工作区内，只排到该工作区其他会话之前，不再被提升到跨工作区的全局置顶分组；多个置顶会话按最近置顶时间排列。状态视图同样只在会话当前的状态分组内置顶，不改变状态分组的优先级。
- 侧栏“显示偏好”可以多选项目范围，空选择表示显示全部项目；项目暂时被主机筛选隐藏或所在主机尚未连接时，保存的项目选择不会被删除，也不会把侧栏错误清空，项目重新出现后会自动恢复筛选。项目分组、状态分组和工作区内置顶会话统一遵循该范围，且不会覆盖工作区与会话的自定义顺序。
- 左侧桌面侧栏采用紧凑父子层级，工作区父行约 32 像素、会话行约 30 像素并缩短组间空白；移动端继续保留原有触控高度。连接多个主机时，远程会话继续显示主机名以便辨识，本机桌面托管连接的会话不重复显示本机主机名；项目分组、状态分组和工作区内置顶会话保持一致。
