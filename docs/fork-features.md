...
本文只记录当前相对原作者已发布版本仍然有效的二开功能，是功能审计和上游情报对照时的唯一差异清单。
...
- 桌面工作区标签使用编辑器式表面层级表达选中状态：活动标签与正文表面连通，未活动标签保留较暗底色，悬停只做轻微提亮，不再使用贯穿标签的高亮横线；多分栏焦点通过活动标签标题的轻微字重变化区分。
...
- 已导入会话仍会被识别并从可导入列表中过滤；Codex 使用“提供方 + 原生会话 ID”作为唯一身份，在导入候选、并发导入、历史记录匹配和客户端会话聚合中防止同一会话重复出现。例外是旧工作目录已经不存在、原生会话已明确报告可用的新目录：该会话会重新出现在导入列表中，用户确认导入后恢复原有 Paseo 会话身份、名称、模型配置和创建时间，并绑定新工作区，不创建重复会话。仍加载着运行时的会话必须先手动释放；不为了迁移目录中断运行中的会话，不按同名文件夹猜测路径，也不在刷新列表时擅自迁移。原生目录未确认或恢复失败时保持明确错误，并恢复原有 Paseo 记录。
...
- Codex 直接在当前线程中原地回退对话，不创建新的线程副本；传统历史按原生回退接口删除末尾回合，分页历史按原生用户消息或客户端消息标识定位所属回合，再使用分页会话的原地回退接口删除该回合及后续回合。定位会遍历完整历史分页，不按正文或界面序号猜测；回退后按响应游标重新读取全部保留内容，不把接口返回的空历史数组误认为整个对话已清空。Codex 的对话回退不会撤销已经写入磁盘的文件修改。
- Codex 明确报告原始记录缩短、历史索引失配时，回退会显示需要先释放运行时并备份修复原生索引的中文提示，保留原生异常作为诊断原因；不自动重复回退，不偷偷分叉或直接改写原始会话文件及提供方数据库。该提示不代表数据已经修复，损坏索引需要独立处理。
- Codex 回合提交时会把 Paseo 客户端消息标识一同交给原生会话持久保存，提交成功后立即记录它与原生用户回合的对应关系；即使用户在原生用户消息事件返回前立刻中断，也能直接回退刚发送的消息，无需重新加载会话。原生消息稍后到达时会并入同一回合，不会导致回退范围多算一次。
...
- 设置页可以配置每次打开老会话加载的最近对话轮数，默认 50 轮；服务端先按用户消息确定最近轮次范围，不把工具调用等时间线记录误算成对话数，再把首屏严格限制为最多 40 条投影记录，避免超长 Codex 会话在载入完成后一次性阻塞整个界面。首屏显示后，可见会话同步器按 40 条一页向前补齐配置要求的轮次，长回合也分批应用；配置范围以外的历史仍由继续上滚或点击尚未加载的索引按页补载。应用重启后从本地缓存副本恢复的会话同样重新权威加载，缓存尾部（仅最后 50 条记录、可能只覆盖一个回合尾部）不会被当作已同步完整历史，也不会用它的陈旧光标走增量路径跳过加载。
...
- 上滑加载旧历史时，加载指示器不占用消息布局；虚拟行在提交阶段同步测量，已收起的中间过程从首帧就按隐藏内容或收起入口的实际高度估算，不再先按完整内容绘制后逐行缩短。分页从一轮对话中间开始时，已完成的历史区域直接收起已加载过程项，不再等待用户消息边界；即使智能体正在处理新回合，旧历史的行高也保持稳定，只有当前正在流式输出的实时区域保持展开。重新测量只补偿视口上方的高度变化，并按包含常驻历史和实时内容的完整对话判断是否接近底部，不把虚拟区末尾误判为对话底部而留下大块空白。补入旧历史前按可见消息身份记录阅读位置，补载后以及首次启用虚拟列表时都恢复同一位置。停在顶部继续上滚仍可加载，无需先向下滚动；补页只有折叠过程、没有新增可见内容时，按权威历史游标的实际推进继续补载，离开加载点后停止，相同游标不会重复自动请求。
...
- 时间线订阅不随应用可见性关闭：切到其他应用或窗口隐藏期间，agent 事件仍持续推送到本地存储；正在初始化或运行的会话即使切到其他对话、工作区或设置页也保持订阅，停止后再按 30 秒宽限期释放。会话重新显示时不论是否仍在宽限期内，都从本地游标增量核对到守护进程最新尾部；已经被回收的会话再次点开时，即使本地已有完整历史，也会主动恢复同一个提供方运行时，而不是只显示旧缓存。应用重新聚焦时先同步排空已经收到但尚未渲染的实时事件，再核对当前可见和运行中会话，避免后台定时器暂停后出现“完成通知已到达但对话仍是旧记录”。发送消息时，守护进程会在启动回合前确保该发送连接收到目标会话的实时过程，不能因可见订阅尚在确认中而漏掉整个回合。Electron 主窗口禁用后台定时器节流，失焦时流式事件与同步逻辑保持实时。
...
- “打开方式”中的 Kiro 使用随桌面端打包的 Kiro 官方应用图标，不再复用命令行终端图标；图标由桌面主进程读取为数据 URL，网页界面不直接访问本机安装目录。
...
- Codex 权限选择器支持“自定义 (config.toml)”模式，实际读取并应用用户配置中的审批策略、审批人、沙盒类型、额外可写目录和工作区网络权限；配置结构非法时明确报错，不会静默退回默认权限。已进入对话的会话（所有提供方）权限选择器与新建会话使用同一套按模式区分的图标：运行时未携带图标的模式按模式标识从提供方目录补齐，不再全部显示为同一个盾牌。
- Host 设置侧栏提供独立的“Codex 服务商”页面，用于保存多个有序的会话级服务商注入配置；该能力不再混在 Agents 页面。每项包含显示名称、`model_provider` 标识、一个或多个模型、API 地址、密钥变量名、运行时环境变量、OpenAI 登录要求、WebSocket 支持及可选的额外 `model_providers` 参数。编辑器使用紧凑的结构化字段、可增删的模型和环境变量行，并只把少见字段留在折叠 JSON 区域；编辑已有条目时会在弹窗显示前一次性回填完整快照，关闭后销毁表单，避免原生输入框忽略异步赋值或把上一次编辑内容带入下一次。所有新建 Codex 会话的输入区都提供独立服务商注入菜单，可在首次启动前先选服务商、再选该服务商下的模型；选择只影响该新会话，不把中转模型写入全局 Codex 偏好。现有 Codex 会话主菜单只显示一个“注入 Codex 服务商”入口，进入后同样分级选择，服务商数量不会拉长主菜单；旧配置没有模型时仍可选择保留会话当前模型。尚未加载的会话会携带所选服务商与模型直接恢复或启动，已加载的会话会先完整释放旧 app-server 再恢复同一原生线程；对话框始终显示会话实际使用的注入模型，即使该模型不在原 Codex 模型目录中也不会回退显示默认模型。选择结果随 Paseo 会话持久化，后续自动恢复继续使用同一配置；配置被删除后会明确报错，不会静默回退到其他服务商或用户全局配置。
...
- 侧栏会话名称前显示当前根智能体的提供方图标，Claude、Codex、OpenCode、OMP、Pi、Copilot 和 ACP 提供方复用统一图标解析规则；项目分组、状态分组和固定会话的行菜单都可复制该智能体在 Claude、Codex 等提供方中的原生会话 ID，便于在对应智能体 CLI 中恢复或定位会话。原生会话 ID 尚未建立时不显示该操作，也不会退回复制 Paseo 内部智能体 ID。

主要涉及：

- `packages/server/src/server/agent/providers/codex-app-server-agent.ts`
- `packages/app/src/codex-provider-injections/`
- `packages/app/src/composer/agent-controls/mode.ts`
...
- `packages/app/src/components/zoomable-viewport/`
...
- 智能体运行期间完整展示思考、工具调用、待办和中间助手消息；回合成功结束后默认收起这些中间过程，只保留模型最后一条助手消息作为最终结论。回合完成事件到达时，客户端无条件把状态归位为空闲（即使本地状态因状态推送丢失而陈旧），确保“完成后收起”不被本地陈旧状态卡住。折叠范围同时遵循用户消息和明确的原生回合边界，自主回合不能把上一轮最终答复当作过程隐藏；历史分页与实时尾部共用折叠归属，临时过程入口不会与正常完成入口重复，窗口从回合中间开始时入口仍放在已加载过程的起点。
- 流式助手正文与推理文字按帧稳定推进，不再直接显示服务端大小不一的合并批次；完整文本始终保留在时间线仓库，回合结束时立即结清尚未显示的尾部，不延迟成功回合的过程收起，也不改变用户消息、steer 追加和提供方回合的既有划分。
- 收起入口位于原过程开始位置，以与正文一致的字号按当前界面语言显示“已处理”及完整回合耗时；中文耗时在数字与单位之间保留空格，避免连续数字和汉字过于拥挤。右侧只保留方向箭头，并通过下方分界线与正文分隔；收起后如果最终结论的首个可见 Markdown 分块是分界线，即使结论被拆成多个流式消息分块，也复用该分界线，不重复绘制相邻横线。展开和收起时仅切换箭头方向，不显示数量标记、选中底色或按压变色。用户可以展开查看全部过程并再次收起。包含最终错误记录的失败回合保持完整展开，避免隐藏失败原因。每条用户消息在展示层都开启新的对话轮次并产生独立的已处理时间与折叠区段，包括通过 `steer` 投递到正在运行智能体的追加要求；底层仍可使用提供方原生追加能力而不中断运行，但追加前后的过程不再合并为一个展示轮次。旧版提供方运行时复用相同回合标识或旧持久化记录缺少归属字段时也遵循同一规则。分页加载从回合中间开始时（该回合的助手消息或用户消息还在更早的未加载历史里），已完成的历史区域把已加载过程项按收起入口处理，直接收起而不再等边界加载；时间线末尾最后一个已完成回合同样适用。打开旧对话时，即使智能体正在处理新回合、初始窗口又只覆盖旧回合尾部，旧过程也会立即收起，无需先回到底部或加载更早分页；当前正在流式输出的实时过程仍保持展开。上下文压缩只是当前回合中的分隔事件：压缩标记到达时会先结清其前方的流式正文并保持时间顺序，压缩后的工具过程即使已进入稳定时间线尾部，也不会在回合仍运行时被误判成已完成历史并隐藏。只有收到真正的回合结束事件后，整轮过程才统一收起。如果最新一页在折叠后短到无法形成有效的向上滚动距离，桌面端会自动逐页补载更早历史，直到可以正常上翻或历史耗尽，不再要求先点击对话索引来解除卡住状态。
- Codex 运行时恢复会把提供方返回的完整助手消息与已经可靠保存的同一消息实时片段对齐，即使两边的消息标识不同、实时正文带有助手消息分界线，也只保留带有原回合归属的实时记录，不再把中间过程和最终回复各插入一份。已经被旧版本写入同一回合的无归属提供方副本，会在下次恢复时自动清理，折叠与展开始终使用同一份内容。
- Codex 每次启动前台回合都使用跨运行时唯一的逻辑回合标识，释放或自动回收运行时后不会重新从本地序号零开始。恢复原生历史时按 Codex 的真实回合结构补全新回合与运行中追加标记，使旧版本已经写入的重复序号也能在重新加载后恢复为正确的折叠与耗时边界。
- Codex 单次补丁修改多个文件时，实时事件和历史恢复都保留完整文件路径列表；工具调用汇总按全部路径去重计数，折叠行展示所有路径，展开详情列出每个文件，不再只显示补丁中的第一个文件。
- Paseo 自有工具调用在二开的原有折叠卡片和移动端详情弹层内显示提示词、关键输入与结果字段，不再要求用户阅读原始 JSON；字段标题使用简体中文，未知的新工具保留完整参数和值作为明确后备。该可读详情不替换回合级过程折叠、完整编辑文件列表、用户字号或可调消息间距。
- 长对话中的助手消息共用同一套 Markdown 解析器，减少重复初始化和常驻对象；应用内所有 Markdown 解析入口统一关闭排版字符替换，助手回复、计划卡片、PR 评论、文件预览和富文本复制都会原样保留 `(c)`、省略号、横线及直引号，避免屏幕内容或复制结果与模型输出、磁盘文件不一致。助手消息继续单独允许本机文件链接，其他界面保持严格链接校验；裸 URL 是否自动链接仍沿用各界面原有行为。
- 恢复持久化旧会话时，在发布可交互状态前主动读取提供方当前上下文用量并与已保存用量合并，不必等待下一次对话才更新输入框旁的上下文进度；圆形进度固定从十二点方向开始顺时针绘制，输入框的发送和停止主操作使用更紧凑的 24 像素按钮。用户可在通用设置关闭上下文圆圈的账户额度探测；关闭后圆圈仍显示当前会话的上下文占用、Token 数和费用，但打开提示卡不会请求或展示服务商账户额度，主机“使用情况”页面不受影响。
- 手机端键盘升降通过同一个布局停靠层同步约束对话正文、插件输入附件和输入框，自动补全浮层会跟随键盘实时调整底边与最大高度；展开长草稿时只压缩可滚动文本区，24 像素发送/停止按钮和其他底部控制始终可见。网页和桌面端发送后继续保持输入焦点，原生端仍按平台习惯收起键盘；切换会话或重启后恢复的长草稿会在首轮布局中立即重新测量高度，不会保留文本却退回单行高度。这些行为不改变二开输入框尺寸、上下文环和用户可调间距。
- Pi 会话提供“完全访问”“每次询问”和“只读”三种权限模式。由于 Pi 核心没有内置权限弹窗或模式接口，Paseo 通过每个 Pi 会话都加载的隔离扩展控制活动工具并把编辑、写入与 Shell 执行确认映射为现有权限卡片；运行中切换会即时更新同一会话，恢复会话时继续沿用已保存模式，未知模式明确报错。
- Claude 使用 Agent SDK 上下文接口，Codex 使用恢复线程上报的最新用量，OpenCode 从原生会话消息读取最近用量，OMP 与 Pi 使用会话统计接口；提供方没有可用快照时保留已保存数据，查询失败会写入错误日志。
- 会话历史同步或订阅被提供方暂时拒绝时，后台重试从 1 秒开始逐步延长到最多 30 秒，避免持续高频请求；错误提示提供立即重试操作，重复点击在本次重试完成前禁用。断线重连、传输模式切换或同步成功后会恢复初始等待时间；首次加载失败同样可以原地重试，不必切换页面。

主要涉及：

- `packages/app/src/agent-stream/layout.ts`
- `packages/app/src/agent-stream/text-reveal.ts`
- `packages/app/src/agent-stream/turn-membership.ts`
- `packages/app/src/agent-stream/view.tsx`
- `packages/app/src/hooks/use-revealed-text.ts`
...
- `packages/server/src/server/agent/agent-manager.ts`
- `packages/server/src/server/agent/history-reconciliation.ts`
- `packages/server/src/server/agent/providers/claude/agent.ts`
- `packages/server/src/server/agent/providers/codex-app-server-agent.ts`
...
### 12. 当前会话和 Claude Code 后台子代理不会被空闲回收误杀

- 任何已连接客户端当前选中或在分栏中可见的会话都会加入空闲运行时回收保护，即使应用窗口暂时隐藏或失去焦点也不会在两分钟后被关闭；回到应用发送消息无需重新启动提供方进程和读取完整历史。会话此前已经被回收时，再次点开会主动恢复运行时并重新确认实时投递。没有客户端查看、没有运行任务的其他空闲会话仍可正常回收，手动“释放运行时”也不受影响。
...
- Claude 原生子代理的实时任务 ID 与父级 `Agent` 工具调用 ID 会归一为同一条记录，任务提示作为子代理时间线首条用户消息，后续推理、工具调用和回复不会被拆到重复行。子代理标题依次使用原生独立名称、任务描述、子代理类型和默认文案，因此没有单独命名的 `general-purpose` 子代理会显示可区分的具体任务。主代理轨道只展示主代理直接创建的子代理；子代理继续创建的下一层代理保留为该子代理时间线中的 `Agent` 调用，不再平铺到主代理轨道。历史重新加载遵循相同层级和身份规则，并在提供方返回子代理历史时先替换旧快照，不会因运行时释放、自动回收或重新打开而把同一批内容重复追加；提供方没有返回子代理历史时仍保留现有快照。
...
- `packages/server/src/server/agent/agent-sdk-types.ts`
- `packages/server/src/server/agent/agent-manager.ts`
- `packages/server/src/server/agent-runtime-protection.ts`
- `packages/server/src/server/bootstrap.ts`
- `packages/server/src/server/session.ts`
- `packages/server/src/server/agent/providers/claude/agent.ts`
- `packages/server/src/server/agent/providers/claude/sidechain-tracker.ts`
...
- 左侧栏、会话列表、智能体输入控件、分栏容器、全局提示接口和终端控制逻辑不再直接订阅整套 Unistyles 主题。普通背景、边框和文字继续由主题样式代理更新；图标、全局滚动条、下拉刷新和终端配色由小型 `withUnistyles` 叶子独立更新。网页端的已挂载样式持续监听运行时主题补丁，启动加载以及切换主题、字体、字号和语法配色时会一次性统一使用当前设置，不会出现部分文字要等到悬停后才改变或同级区域混用新旧字号；全局滚动条使用独立的主题变量并提供暗色回退，不会因关闭 Unistyles CSS 变量模式而退回白色原生轨道，同时不连带重跑工作区投影、列表分组、终端流处理、提示接口和输入控件状态。
...
- 提交说明输入支持多行自适应高度，删空后立即恢复单行高度；`Ctrl/Cmd + Enter` 提交，`Alt + ↑/↓` 浏览本次面板会话的提交说明历史。输入框不显示占位文案，Web 端只显示单层 VS Code 式焦点边框，输入高度不会被浏览器默认多行尺寸撑大。输入框右侧可调用独立配置的内部智能体读取当前完整未提交差异并生成结构化提交说明，生成期间显示紧凑加载状态，失败时保留原输入并在原位显示错误；任务完成、失败或连接断开后，Claude、Codex、OpenCode、Pi、Grok 清除 Paseo 临时记录并按各自原生能力删除或归档临时会话，其他 Provider 归档临时会话。Grok 临时任务会先关闭 ACP 会话并终止其进程，再使用该 Provider 配置的同一启动器和环境执行原生删除，不再残留未命名历史；删除失败会作为任务清理错误报告。提交按钮采用 VS Code 式分裂按钮：有暂存文件时主按钮只提交暂存区，没有暂存文件时主按钮提交全部更改；下拉菜单始终明确提供“提交暂存区”和“提交全部更改”，主按钮与下拉段共享一致的悬停、展开和禁用层级。提交成功后只同步刷新本地 Git 快照，立即清空说明并恢复操作；代码托管平台状态、提交图表和其他查询继续在后台刷新，刷新失败会记录完整错误，不会把已经成功的提交误报为失败。提交失败时保留说明并显示错误。
- 工作区干净时，提交主按钮继续按仓库状态切换为同步更改或发布分支；同步显示落后与领先计数。Fetch、Refresh、Pull、Push、Sync 都执行真实 Git 操作并显示进行中、成功或失败状态，新写操作不会用旧接口拼接残缺降级路径。
- 提交图表默认展开为 520 像素高度，可拖动顶部分隔线在 140 至 720 像素及当前视口 70% 范围内调整，拖动结束后持久记忆用户高度；收起图表时只保留 22 像素标题行。标题和提交行统一为 22 像素，提交正文使用与上方更改列表一致的 13 像素字号，每行只为自身实际泳道保留图谱宽度；主行中的提交说明优先向右省略，分支、远端和标签引用徽标保持可见。悬停详情按 VS Code 的紧凑 Markdown 节奏展示作者与时间、完整说明、文件增删统计、引用和提交操作，卡片字号比面板正文小一号，宽度随内容自适应，分支、远端和标签引用始终单行展示，完整说明中的“-”分点渲染为实心圆点，段落与分隔线使用 4 像素间距，引用标签保持约 18 像素高度，当前分支与本地分支图标统一为 10 像素，并支持复制完整提交标识和打开对应托管平台提交页面。
...
- 托管平台的当前拉取请求始终跟随工作区实际检出的分支：持久化的拉取请求身份只有在仍绑定当前本地分支时才复用，Paseo 内重命名分支会同步迁移绑定，智能体直接切换或重命名分支后则从当前 Git 配置重新识别。合并后自动归档只会响应守护进程实际观察到的同一拉取请求从“打开”变为“已合并”，归档前再次读取权威状态；首次打开已经合并的旧拉取请求、切换到另一个已合并请求或手动恢复已归档工作区都不会触发误归档。GitHub 状态轮询按主机共享 5 秒对齐的全局调度，每批最多查询 25 个工作区，并在账户额度降到保留线后统一暂停到重置时间；首轮等待仓库标识解析完成再合并查询，单个工作区解析缓慢不会阻塞其他工作区。批量响应、主机识别或别名解析失败会保持明确错误，不会伪装成“没有拉取请求”；已关闭或已合并请求的检查结果不会被永久冻结，仍按慢速周期读取，从而继续反映同一提交上后来重新运行的检查。
- 点击提交主行直接展开或收起文件子行，同时保持选择背景；行末箭头提供相同的显式展开入口。桌面端悬停或选中时显示打开完整差异的行内操作，移动端始终显示。提交右键或长按菜单可启动独立配置的内部智能体审查该提交；审查过程由应用级宿主持有，在工作面右下角的浮动窗口中实时展示完整工具调用和最终回复，默认尺寸约为 420×600 像素，支持拖动、四角缩放、收起、展开和关闭，不受 Git 侧栏宽度限制。切换会话、Git 标签或设置页面不会关闭审查；收起时固定使用独立的 360×34 像素紧凑标题条，不沿用用户调整后的展开宽度。关闭运行中窗口或连接断开时会先停止并只清理本次临时智能体，清理失败则保留窗口并显示错误。启动另一提交的审查前会先清理当前审查；临时智能体不进入普通会话列表。权限响应由守护进程统一发送明确的结束事件；提供方未主动发送时由守护进程在状态刷新后补齐，批准按钮和权限卡片不会因缺少提供方事件而持续等待。展开后的文件沿用图谱泳道占位、Material 文件图标、灰色目录和状态字母，点击文件会打开该提交的完整差异并定位到对应文件。提交历史每页加载 40 条，滚动到底继续加载。
- 暂存、取消暂存和放弃更改使用带方向后缀的点分 RPC，并通过主机能力集中门控；旧主机收到明确的更新提示，不尝试以旧差异接口模拟写操作。协议新增字段保持可选，旧客户端和旧守护进程仍能互相解析消息。
- 面板复用现有跨平台组件、主题语义色和文件图标体系，桌面、网页和移动端保持同一操作语义；主机智能体设置分别配置提交说明生成与提交审查所用的智能体、模型、该智能体原生提供的权限选项、思考强度和完整可编辑的任务提示词，两项任务互不覆盖；服务端只注入目标提交标识，不再在用户提示词前隐藏追加审查规则。选择 OpenCode 时还可独立开启自动批准，启动临时智能体时通过 OpenCode 原生 `auto_accept` 功能值生效，不覆盖已选的 `Build`、`Plan` 或自定义模式；切换到其他智能体时不会传递该功能值。空白提示词按任务恢复为预置内容；提示词编辑框在网页/桌面端支持纵向拖拽调整高度；所有新增界面文案在全部语言资源中保持同构。
...
- `packages/app/src/git/commit-message-text.ts`
...
- Electron 主窗口只允许打包应用的 `paseo://app` 来源或开发环境配置的精确来源留在宿主窗口中，HTTP(S) 链接交给系统浏览器，文件、脚本和其他协议导航直接阻断；导航重定向和新窗口使用同一规则。主窗口的通用命令、文件、守护进程、更新安装、CLI、技能、窗口、菜单、通知、对话框、编辑器和浏览器控制 IPC 只接受该受信任页面主框架的请求，嵌套框架、内置浏览器网页和意外加载的远程页面不能调用本机高权限能力。内置浏览器继续使用独立沙箱和低权限 preload，不受主窗口宿主桥接影响。
- 工作区目录首次加载只返回守护进程已有的 Git 缓存，不再为列表中最多 200 个未打开工作区同时注册 Git 观察器；进入具体工作区并读取其本地 Git 状态后才注册该目录的实时观察，当前工作区的分支、差异和状态推送保持不变。
- 桌面端支持在设置中选择退出应用时停止桌面托管的守护进程，但二开默认仍保持守护进程运行。已有的退出选择不会因旧设置缺少默认值迁移标记而被重置：选择退出时停止后，仍会执行托管守护进程的完整停止流程；选择保持运行时，再次打开桌面端会复用已有守护进程，不重复启动。独立启动、非桌面托管的守护进程不受桌面退出操作影响。
...
- `packages/desktop/src/security/trusted-renderer.ts`
...
- 从已经明确项目和主机的侧栏入口新建会话时，先显示不写入服务端记录的完整草稿工作区外壳：顶部草稿标签、工作区菜单、终端/浏览器入口以及右侧文件和 Git 面板无需先发送一条消息即可使用，中央继续沿用原有居中的空白会话输入区。尚未发送首条消息的新建会话按“主机 + 项目目录”在后台常驻，每个项目只有一个：离开新建页、关闭草稿标签或切换到其他项目后再次点击新建，都会回到同一个草稿，保留输入内容和已打开的文件；草稿标签栏的“+”也只切回该草稿。该草稿不出现在左侧列表，只保存在内存中，退出应用后不再恢复。从右侧文件面板打开的文件在外壳内作为附加正文标签显示，草稿标签和输入区保持不变，可随时切回；打开文件不会创建工作区，也不会让会话出现在左侧。关闭文件标签和批量关闭只作用于文件，关闭存在未保存修改的文件或离开新建页前会先确认。发送首条消息或主动创建空工作区后，草稿才正式成为会话，已打开的文件按原顺序带入新工作区，下次新建会得到新的草稿；主动打开终端或浏览器仍需要持久工作区，会先创建工作区再进入。全局“新建工作区”入口没有明确项目时仍保留项目选择步骤，分叉到新会话等显式携带草稿的入口继续使用各自独立的草稿，不被项目常驻草稿抢占。
- 工作区支持按加入 Paseo 的先后、名称和自定义顺序排列，默认沿用加入 Paseo 的先后。切换到名称排序时禁用拖动，拖动加入时间顺序中的工作区后自动进入自定义顺序。
- 工作区首次被 Paseo 发现的顺序和手工拖动顺序分别持久化；新加入的工作区目录在“加入 Paseo 时间”和手工拖动后的“自定义顺序”中都直接置顶，同时保留已有目录的相对顺序。未手工排序的工作区内部会话按加入 Paseo 的时间从新到旧排列；已经因首条消息或工作区操作而正式创建的空会话也直接出现在顶部，但只存在于客户端的新建草稿不会提前占据左侧列表。排序不受标题、最近活动或请求返回顺序影响。加入时间使用主机持久化的工作区创建时间，创建响应、目录快照、实时更新和客户端副本均保留该时间；相同时间按稳定标识排列，改名、继续对话和重新打开应用不会让会话跳位。拖动会话后持久化该工作区的自定义顺序，之后发现的新会话仍插入顶部并保留已知会话的手工相对顺序。会话菜单同时提供两种互斥置顶：全局置顶会把会话提升到跨工作区的“已置顶”分组，工作区内置顶则让会话留在原工作区并排到同组其他会话之前；切换范围时会先取消另一种置顶，两个范围都按最近置顶时间排列。状态视图中的全局置顶同样只显示一次，工作区内置顶不改变状态分组优先级。
- 侧栏“显示偏好”可以多选项目范围，空选择表示显示全部项目；项目暂时被主机筛选隐藏或所在主机尚未连接时，保存的项目选择不会被删除，也不会把侧栏错误清空，项目重新出现后会自动恢复筛选。项目分组、状态分组和两种置顶会话统一遵循该范围，且不会覆盖工作区与会话的自定义顺序。
- 左侧桌面侧栏采用紧凑父子层级，工作区父行约 32 像素、会话行约 30 像素并缩短组间空白；移动端继续保留原有触控高度。连接多个主机时，远程会话继续显示主机名以便辨识，本机桌面托管连接的会话不重复显示本机主机名；项目分组、状态分组和两种置顶会话保持一致。
...
- `packages/app/src/screens/new-workspace-draft-shell-model.ts`
- `packages/app/src/screens/new-workspace/draft-shell-store.ts`
- `packages/app/src/stores/draft-keys.ts`
- `packages/app/src/stores/draft-store/`
- `packages/app/src/stores/session-store.ts`
- `packages/app/src/runtime/replica-cache/index.ts`
- `packages/server/src/server/session.ts`
...
- 当前会话命名失败时保留提示词生成的临时标题或占位分支，不切换到其他 Provider。提交说明、拉取请求文本等其他元数据任务仍沿用可配置的回退顺序；单独创建且尚无智能体的工作区也继续使用原有元数据生成规则。结构化元数据任务的临时 Grok 会话在使用后通过原生命令删除；关闭或删除本地状态失败会显式中止任务，不会被当成模型失败继续创建更多回退会话。
...
- 首次订阅和断线重订阅确认后都会立即执行一次权威刷新，覆盖观察器建立期间恰好新增文件的竞态；自动刷新在后台更新目录，不切换目录箭头或列表加载状态，目录图标槽保持固定尺寸。每条客户端目录订阅拥有独立的串行刷新队列：刷新期间到达的新变化会在本轮结束后继续处理，旧客户端或旧工作区订阅即使仍有请求未返回，也不能挡住新订阅；已经失效的旧请求不会再覆盖当前目录。客户端断线重连后使用原订阅标识恢复目录观察；关闭面板、切换工作区和释放连接时会取消订阅，服务端等待递归观察器完成所有扫描、待发事件和原生句柄释放后再确认。观察启动或运行失败会返回明确错误并记录完整异常，不会静默退回轮询。
...
### 23. 插件体系适配二开正文标签、输入框和外观设置

- 保留上游从本地目录或 Git 仓库安装、检查、原子更新、启用、禁用、重新加载和移除受信任插件的完整生命周期；插件仍可贡献服务端 RPC、附件来源、时间线转换与渲染、主题、工作区面板、命令面板操作、宿主弹窗、图标、提示条和输入框上下文标签。
- 插件工作区面板统一进入二开的正文标签系统。插件声明在 Explorer 打开的请求也映射为正文标签，不引入上游 Side 面板、Explorer 或 Git 面板结构；右侧 Git 和文件面板继续由二开独立维护。
- 插件全局界面和工作区面板可以请求宿主打开当前主机上的智能体或工作区，但只接收由宿主管理的导航能力，不直接访问 Expo Router 或工作区布局仓库；工作区仍进入二开的正文标签，插件不能借此恢复上游 Side/Explorer 面板。
- 插件主题先使用上游语义色构建，再叠加二开的黑白 Dark、界面字体、字号和语法高亮设置。切换主题、字体或字号时，已挂载的插件界面与宿主界面一起更新。
- 插件时间线只改写客户端投影结果，不修改服务端规范时间线；历史重新加载、本地副本恢复和实时事件都经过同一套重投影，继续遵守二开的完整历史索引、正文内存预算、虚拟列表锚定和成功回合收起规则。
- 插件输入框标签接入二开现有输入框上方区域，没有标签时不占空间，也不恢复上游已经删除的输入框轨道和智能体轨道文件。卸载、禁用、替换、宿主断线或应用卸载会清理该插件的标签、命令、面板和查询状态，不影响其他插件或普通会话。

主要涉及：

- `packages/app/src/plugins/`
- `packages/app/src/panels/agent-panel.tsx`
- `packages/app/src/plugins/workspace-panels/`
- `packages/app/src/plugins/composer-pills/`
- `packages/app/src/plugins/timeline/`
- `packages/app/src/appearance/`
- `packages/plugin/`
...
- 权限分类覆盖二开新增的运行时释放、目录观察、Git 暂存/撤销与 AI 提交说明/审查 RPC；参考上游权限表变化时必须继续保留这些操作，不能因上游不存在对应消息而删掉。
...
- 后端发现和普通脚本成功不代表浏览器认证成功；实际连接仍依赖当前 Codex 数据目录中的有效 ChatGPT 登录及其策略服务网络。Paseo 为桌面工具生成独立认证启动器，只让 Computer Use 和 Chrome 的内部认证进程临时使用官方 `openai` 服务商读取已有账号令牌，正文会话继续使用用户选择的中转或自定义服务商。桌面工具优先继承显式代理环境；没有显式代理时自动读取 Windows 当前用户启用的系统代理，避免 Paseo 能运行中转模型但官方工具无法访问 ChatGPT。启动器随实际原生 CLI 路径更新，不修改全局 `config.toml`，不复制凭据，也不把中转 API Key 当作账号登录态。
- 配置覆盖仅作用于当前 Codex 运行实例；保留原有工具限制，并根据实际启用的 Computer Use 与 Chrome 插件生成统一工具界面，即使官方模板只声明浏览器也不会漏掉原生应用控制。不改写用户全局配置、官方插件缓存或权限。用户明确覆盖相关 MCP/插件、禁用服务，以及非 Windows 或非 App 管道型配置时，不自动接管。
...
- `packages/server/scripts/build-codex-desktop-host.mjs`
- `packages/desktop/electron-builder.yml`
- `packages/desktop/scripts/after-pack.js`
- `packages/desktop/src/daemon/codex-desktop-host.test.ts`
- `packages/server/src/server/agent/providers/codex-app-server-agent.ts`

### 26. Agent 配置支持自动模型名称与拖拽排序

- Agent 配置的名称是可选自定义标签。名称留空时，设置列表和模型选择器实时使用当前模型的友好名称；模型目录暂不可用或模型已经下线时回退到保存的模型标识，仍无模型时回退到 Provider 名称。切换模型会同步改变自动名称，不会把生成名称写回配置，也不会在摘要里重复显示同一个模型。
- 配置列表使用明确的拖拽抓手调整顺序，Web 支持鼠标、触控和键盘拖拽，Native 长按抓手拖动；屏幕阅读器仍可通过上移、下移动作排序。拖动结束后立即显示新顺序并整体保存到 Host，保存失败会恢复原顺序并显示错误，不会只在当前客户端临时生效。
- `list_profiles` 对未命名配置使用保存的模型标识作为名称，使编排智能体看到的配置仍然可辨认；显式设置的名称保持不变。
