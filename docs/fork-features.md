...
- 收起入口位于原过程开始位置，以与正文一致的字号按当前界面语言显示“已处理”及完整回合耗时；中文耗时在数字与单位之间保留空格，避免连续数字和汉字过于拥挤。右侧只保留方向箭头，并通过下方分界线与正文分隔；收起后如果最终结论的首个可见 Markdown 分块是分界线，即使结论被拆成多个流式消息分块，也复用该分界线，不重复绘制相邻横线。展开和收起时仅切换箭头方向，不显示数量标记、选中底色或按压变色。用户可以展开查看全部过程并再次收起。包含最终错误记录的失败回合保持完整展开，避免隐藏失败原因。用户消息会明确保留“开启新回合”或“追加到运行中回合”的归属：新回合始终产生独立的已处理时间与折叠区段，即使旧版提供方运行时曾复用相同回合标识；运行中追加仍留在原回合内。旧持久化记录缺少该归属字段时，用户消息默认开启新回合，只有明确标记为运行中追加的消息才沿用上一回合，避免仅因复用标识把两次独立对话合并。分页加载从回合中间开始时（该回合的助手消息或用户消息还在更早的未加载历史里），已完成的历史区域把已加载过程项按收起入口处理，直接收起而不再等边界加载；时间线末尾最后一个已完成回合同样适用。打开旧对话时，即使智能体正在处理新回合、初始窗口又只覆盖旧回合尾部，旧过程也会立即收起，无需先回到底部或加载更早分页；当前正在流式输出的实时过程仍保持展开。
