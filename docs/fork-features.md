...
- 刻度之间使用固定像素间距紧凑排列，不随会话长度均摊到整条轨道：历史很少时刻度不会被拉开，可用高度不足时先减少刻度数量而不是压缩间距。固定间距同时让每个刻度落在整像素上，配合统一的刻度高度消除半像素抗锯齿造成的粗细不均；当前所在刻度通过颜色和波峰长度区分，不靠加高刻度。
...
- 上滑加载旧历史时，加载指示器不占用消息布局；虚拟行在提交阶段同步测量，不再先按估算高度绘制一帧再纠正，避免分页处出现明显的上下跳动。重新测量只补偿视口上方的高度变化。
