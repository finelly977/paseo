...
- 托管平台的当前拉取请求始终跟随工作区实际检出的分支：持久化的拉取请求身份只有在仍绑定当前本地分支时才复用，Paseo 内重命名分支会同步迁移绑定，智能体直接切换或重命名分支后则从当前 Git 配置重新识别。合并后自动归档只会响应守护进程实际观察到的同一拉取请求从“打开”变为“已合并”，归档前再次读取权威状态；首次打开已经合并的旧拉取请求、切换到另一个已合并请求或手动恢复已归档工作区都不会触发误归档。
