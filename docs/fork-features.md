...
- 工作区首次被 Paseo 发现的顺序和手工拖动顺序分别持久化；未手工排序的工作区内部会话按加入 Paseo 的时间从新到旧排列，新会话即使尚未发送消息也直接出现在顶部，不受标题、最近活动或请求返回顺序影响。加入时间使用主机持久化的工作区创建时间，创建响应、目录快照、实时更新和客户端副本均保留该时间；相同时间按稳定标识排列，改名、继续对话和重新打开应用不会让会话跳位。拖动会话后持久化该工作区的自定义顺序，之后发现的新会话仍插入顶部并保留已知会话的手工相对顺序；置顶与项目、主机筛选规则不变。
