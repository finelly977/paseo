...
- 面板复用现有跨平台组件、主题语义色和文件图标体系，桌面、网页和移动端保持同一操作语义；主机智能体设置分别配置提交说明生成与提交审查所用的智能体、模型、该智能体原生提供的权限选项、思考强度和预置且可编辑的任务提示词，两项任务互不覆盖；空白提示词按任务恢复为预置内容；所有新增界面文案在全部语言资源中保持同构。
