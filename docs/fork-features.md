...
- 上滑加载旧历史时，加载指示器不占用消息布局；虚拟行在提交阶段同步测量，已收起的中间过程从首帧就按隐藏内容或收起入口的实际高度估算，不再先按完整内容绘制后逐行缩短。分页从一轮对话中间开始时，已完成的历史区域直接收起已加载过程项，不再等待用户消息边界；即使智能体正在处理新回合，旧历史的行高也保持稳定，只有当前正在流式输出的实时区域保持展开。重新测量只补偿视口上方的高度变化。
