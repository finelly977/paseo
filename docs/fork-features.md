...
- 时间线订阅不随应用可见性关闭：切到其他应用或窗口隐藏期间，agent 事件仍持续推送到本地存储；正在初始化或运行的会话即使切到其他对话、工作区或设置页也保持订阅，停止后再按 30 秒宽限期释放。会话重新显示时不论是否仍在宽限期内，都从本地游标增量核对到守护进程最新尾部；应用重新聚焦时也核对当前可见和运行中会话，避免“完成通知已到达但对话仍是旧记录”。Electron 主窗口禁用后台定时器节流，失焦时流式事件与同步逻辑保持实时。
...
- 打开右侧文件面板时只为当前工作区根目录建立一次递归观察；新增、修改、删除和重命名事件会在写入停止 150 毫秒后合并刷新，持续写入时最长每 1 秒刷新一次，避免短时间反复重绘又不会长期停留在旧列表。随后重新读取根目录和当前仍展开的目录，不为每个子目录分别建立观察器，也不使用轮询。
- 刷新会先读取父目录再读取可见的展开子目录；已经删除或重命名的旧路径不会继续请求，其展开记忆及下级路径会一并移除。关闭隐藏文件时不会为隐藏目录执行无用读取，文件大小和修改时间也会随外部编辑更新。
- 首次订阅和断线重订阅确认后都会立即执行一次权威刷新，覆盖观察器建立期间恰好新增文件的竞态；自动刷新在后台更新目录，不切换目录箭头或列表加载状态，目录图标槽保持固定尺寸。客户端断线重连后使用原订阅标识恢复目录观察；关闭面板、切换工作区和释放连接时会取消订阅，服务端等待递归观察器完成所有扫描、待发事件和原生句柄释放后再确认。观察启动或运行失败会返回明确错误并记录完整异常，不会静默退回轮询。
