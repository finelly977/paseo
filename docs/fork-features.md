...
- 左侧桌面侧栏采用紧凑父子层级，工作区父行约 32 像素、会话行约 30 像素并缩短组间空白；移动端继续保留原有触控高度。连接多个主机时，远程会话继续显示主机名以便辨识，本机桌面托管连接的会话不重复显示本机主机名；项目分组、状态分组和固定会话保持一致。
