...
- 提交图表默认展开并占用变更列表下方的全部剩余空间；变更列表有内容时最多占面板的 42%，工作区干净时不再保留空白高度。图表不再使用固定像素高度或拖动柄。标题和提交行统一为 22 像素，主行优先显示提交说明、灰色作者和带分支、远端、标签图标的实底引用徽标；完整说明、时间、提交标识与增删统计放在悬停信息中。
...
- 点击提交行会保持选择背景，展开按钮单独控制文件子行；桌面端悬停或选中时显示打开完整差异的行内操作，移动端始终显示。展开后的文件沿用图谱泳道占位、Material 文件图标、灰色目录和状态字母，点击文件会打开该提交的完整差异并定位到对应文件。提交历史每页加载 40 条，滚动到底继续加载。
