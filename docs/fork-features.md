...
- 所有刻度按整像素定位并共用同一条基线，粗细保持一致；当前所在刻度通过颜色和波峰长度区分，不靠加高刻度，避免半像素抗锯齿造成横线粗细不均。
...
- 进入设置页时保留左侧工作区侧栏的挂载状态，仅将其隐藏。此前设置路由会整体卸载侧栏，返回对话页时需要重新构建工作区投影，造成一到两秒卡顿；侧栏在隐藏期间保持非活动状态，不执行订阅和动画工作。
