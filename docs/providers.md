...
Pi core deliberately has no built-in permission popups or selectable permission modes. Paseo supplies `full`, `ask`, and `read-only` modes through its generated integration extension. `full` preserves every tool Pi loaded, `ask` keeps those tools but confirms `edit`, `write`, `bash`, and `powershell` through Pi's RPC extension UI, and `read-only` limits the active set to `read`, `grep`, `find`, and `ls`. The selected mode is persisted in the Paseo agent handle and can be changed in a running session through the private `paseo_set_permission_mode` extension command. Do not present Pi's project-resource trust flags (`--approve` and `--no-approve`) as permission modes; they only decide whether project-local Pi configuration is loaded.
...
运行中追加是可选的 `AgentSession.steerActiveTurn` 操作。管理器负责把追加要求严格准入到当前前台回合、创建权威用户消息并对齐提供方回显；只有适配器明确返回 `unavailable` 时，才回到正常的“中断后重新发送”路径。适配器报错表示消息去向不再确定，必须直接向上抛出，禁止擅自中断或重试。Codex 使用带预期回合标识和 Paseo 客户端消息标识的 `turn/steer`；Claude 把已准入消息推入当前 SDK 查询输入，独立控制命令仍不支持追加；OpenCode 使用由 OpenCode 自己生成消息标识的 `session/prompt_async`，服务端会在忙碌期间排队，并让同一 Paseo 回合的下一次模型调用读取该消息。原生会话不存在时返回 `unavailable`，再由管理器走正常的中断回退。
