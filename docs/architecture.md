...
> **桌面宿主信任边界。** 主窗口只加载打包态的 `paseo://app` 或开发态配置的精确来源。页面导航和重定向离开该来源时会被阻断；HTTP(S) 链接改由系统浏览器打开，Electron 子窗口不会继承宿主 preload。所有由 `window.paseoDesktop` 暴露的宿主 IPC 都在主进程再次核对调用者必须是受信任来源的顶层主框架，不能依赖渲染器自行约束。内置浏览器 guest 使用独立沙箱、持久化分区和仅处理键盘边界的 preload，不具备宿主桥接能力。
>
