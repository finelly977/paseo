...
- Paseo 为桌面工具生成独立的只读认证启动器。该启动器只在 Computer Use 和 Chrome 的内部认证进程中把 `model_provider` 临时设为官方 `openai`，使其从当前 `CODEX_HOME` 读取已经存在的 ChatGPT 登录态；正文会话仍使用用户选择的中转或自定义模型服务商。启动器按实际原生 CLI 路径生成到 Paseo 运行目录，Codex 更新后自动换用新文件，不修改全局 `config.toml`，也不复制或另存账号凭据。
...
- 会话配置只替换 Node REPL 的环境变量。统一工具插件的原 MCP 服务在该会话中关闭，并按原工具限制注册同名服务；可用界面根据实际启用的 Computer Use 与 Chrome 插件组合生成，不会因为官方统一插件模板只声明浏览器而漏掉原生应用控制。不改写全局 `config.toml` 或官方插件缓存；明确自定义相关 MCP 或插件覆盖的会话保留用户配置，不自动接管。
...
**认证边界：** Chrome 后端发现成功、工具目录可见或普通 JavaScript 执行成功，都不代表实际浏览器连接已经可用。官方桌面工具需要当前 `CODEX_HOME` 中有效的 ChatGPT 登录态，并需要访问 ChatGPT 与策略服务；中转站 API Key 不能代替该登录。Paseo 会把桌面工具的认证查询与正文会话的模型路由隔离，并优先沿用进程已有的代理环境；未显式配置代理时，Windows 会话自动读取当前用户已启用的系统代理并只注入桌面工具运行环境。因此中转模型和账号功能可以共存，也不需要为桌面工具单独手工复制代理设置。Paseo 不会复制凭据、伪造登录状态或替用户完成首次登录；账号登录失效时仍需通过官方支持的流程更新同一 `CODEX_HOME`，之后无需为了日常使用而保持 Codex App 打开。
